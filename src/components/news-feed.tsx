            ...
            <FormField
              ...
              render={({ field: { onChange, value, ...rest } }) => (
                <FormItem>
                  ...
                  <DialogDescription>
                    {editingPost ? "Upload a new image to replace the current one." : "Optionally, upload an image for the news post."}
                  </DialogDescription>
                  <FormControl>
                    <Input
                      type="file"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <DialogDescription>Provide a link to a video (e.g., YouTube, Vimeo).</DialogDescription>
                  <FormControl>
  ...
  React.useEffect(() => {
    const fetchPosts = async () => {
      ...
      if (result.success && result.data) {
        ...
        setPermissions(result.data.user_info?.permissions || {});
      } else {
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Manage company news and hiring updates.</CardDescription>
            </div>
          ...
          {posts.length > 0 ? (
            posts.map((post) => (
              <Card key={post.id} className="overflow-hidden flex flex-col">
                <CardHeader className="flex flex-row items-center gap-4">
                  <Avatar>
                    <AvatarImage src={post.author_avatar} alt={post.company_name} data-ai-hint="company logo" />
                  ...
                  <div className="flex items-center gap-1">
                    {permissions.can_edit_posts && (
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleOpenFormDialog(post)}>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Avatar className="h-8 w-8">
                      <AvatarImage src={permissions.avatar} data-ai-hint="person portrait" />
                      <AvatarFallback>{permissions.email?.[0]?.toUpperCase()}</AvatarFallback>
                    </Avatar>
                    <div className="relative flex-1">
                      <Input placeholder="Write a comment..." className="pr-10 rounded-full bg-muted" />
            ...
            <div className="md:col-span-1 max-w-2xl mx-auto">
              <Card>
                <CardContent className="p-0">
                  <div className="flex flex-col items-center justify-center text-center p-10 border-2 border-dashed rounded-lg">
                    <CardTitle>No News Yet</CardTitle>
                    <CardDescription className="mt-2">Click "Create Post" to share the first update.</CardDescription>
                  </div>
                </CardContent>
              </Card>
            </div>
