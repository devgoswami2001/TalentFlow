...
const Section: React.FC<{ icon: React.ElementType; title: string; children: React.ReactNode, className?: string }> =
      ...
      <div className="container mx-auto max-w-6xl py-12 px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="print:hidden flex justify-end gap-3">
          <Button onClick={handlePrint}>
            <FileText className="mr-2" /> Print Report
          </Button>
          {/* {resume.resume_pdf && (
            <Button variant="outline" asChild>
              <a href={`${apiBaseUrl}${resume.resume_pdf}`} target="_blank" rel="noreferrer">
                <ArrowDown className="mr-2" /> Download Resume
              </a>
            </Button>
          )} */}
        </div>
        ...
        <Card className="overflow-hidden print-card">
          ...
          <CardContent className="p-6 flex flex-col md:flex-row items-center md:items-end gap-6 -mt-20 relative">
            <Avatar className="w-32 h-32 border-4 border-background bg-background shadow-lg">
              <AvatarImage
                src={`${apiBaseUrl}${profile.profile_picture}`}
            ...
            <div className="flex-grow text-center md:text-left pt-4">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-2 text-sm text-muted-foreground mt-3">
                <a href={`mailto:${profile.email}`} className="flex items-center gap-2 hover:text-primary">
                  <Mail className="w-4 h-4" />{application.applicant_email}
                </a>
                {profile.phone_number && (
                  <a href={`tel:${profile.phone_number}`} className="flex items-center gap-2 hover:text-primary">
            ...
            <div className="shrink-0 text-center md:text-right">
              ...
              <p className="font-semibold text-lg">
                {format(parseISO(application.applied_at), "PPP")}
              </p>
              <Badge
                variant={
                  ai_analysis.fit_level === 'good' || ai_analysis.fit_level === 'excellent'
                    ? 'default'
                    : 'secondary'
                }
        ...
        <Section icon={Bot} title="AI Analysis Summary" className="print-card">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 flex flex-col items-center justify-center text-center p-6 bg-muted/50 rounded-lg border">
              ...
              <p className="text-8xl font-bold font-headline text-primary my-2 tracking-tighter">
                {ai_analysis.score_breakdown.overall_fit.toFixed(0)}%
              </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <h4 className="font-semibold text-lg mb-3 flex items-center gap-2">
                <ThumbsDown className="w-5 h-5 text-amber-500" />Weaknesses & Missing Skills
              </h4>
              <div className="flex flex-wrap gap-2">
                {ai_analysis.weaknesses?.map((w) => (
                  <Badge key={w} variant="outline">{w}</Badge>
                ))}
                {ai_analysis.missing_skills?.map((s) => (
                  <Badge key={s} variant="destructive" className="font-normal">{s}</Badge>
                ))}
                {(ai_analysis.weaknesses?.length ?? 0) === 0 &&
                  (ai_analysis.missing_skills?.length ?? 0) === 0 && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            <Section icon={BrainCircuit} title="Technical Skills" className="print-card">
              <div className="flex flex-wrap gap-2">
                {resume.skills_data?.technical?.map((skill) => (
            ...
            <Section icon={Lightbulb} title="AI Interview Guidance" className="print-card">
              ...
              <ul className="list-decimal pl-5 space-y-3 text-sm text-muted-foreground">
                {ai_analysis.suggested_interview_questions?.map((q) => (
                  <li key={q}>{q}</li>
                ))}
              </ul>
            </Section>
